package BankerAlgorithm;

import java.util.Scanner;

public class BankerClass {
	int[] Available = {10,8,7};
    int[][] Max = new int[3][3];
    int[][] Alloction = new int[3][3];
    int[][] Need = new int[3][3];
    int[][] Request = new int[3][3];
    int[] Work = new int[3];
    
    int num = 0;
    Scanner scanner = new Scanner(System.in);
    
	public BankerClass() {}
	
	
	//设置各初始系统变量 并判断是否处于安全状态
	public void setSystemVariable(){
		setMax();
		setAlloction();
		printSystemVariable();
		SecurityAlgorithm();
	}

	//设置请求资源量request
	public void setRequest(){
		System.out.println("请输入请求资源的进程编号：");
		num = scanner.nextInt(); 
		System.out.println("请输入请求各资源的数量：");
		for(int j=0; j<3; j++){
			Request[num][j] = scanner.nextInt();
		}
		System.out.println("进程P"+num+"对各资源请求Request：（"+Request[num][0]+","+Request[num][1]+","+Request[num][2]+").");
		BankerAlgorithm();
	}
	
	
	//银行家算法
	private void BankerAlgorithm() {
		boolean T = true;
		if(Request[num][0] <= Need[num][0] && Request[num][1] <= Need[num][1] && Request[num][2] <= Need[num][2]){
			if(Request[num][0] <= Available[0] && Request[num][1] <= Available[1] && Request[num][2] <= Available[2]){
				for(int i=0; i<3; i++){
					Available[i] -= Request[num][i];
					Alloction[num][i] += Request[num][i];
					Need[num][i] -= Request[num][i];
				}
			}else {
				System.out.println("当前没有足够的资源，进程P"+num+"需等待.");
				T = false;
			}
		}else {
			System.out.println("进程P"+num+"请求已经超出最大需求量Need.");
			T = false;
		}
		if (T == true) {
			printSystemVariable();
			System.out.println("现在进入安全算法：");
			SecurityAlgorithm();
			
		}
		
		
	}


	//设置Max矩阵
    private void setMax() {
		System.out.println("请设置各进程的最大需求矩阵Max: ");
		for(int i=0; i<3; i++){
			System.out.println("请输入进程P"+i+"的最大资源需求量是：");
			for(int j=0; j<3; j++){
				Max[i][j] = scanner.nextInt();
			}
		}
	}
	
    //安全算法
	private void SecurityAlgorithm() {
		boolean[] Finish = {false,false,false};//初始化
		int count = 0;//完成进程数
		int circle =0; //循环圈数
		int[] S = new int[3];//安全序列
		//设置工作向量
		for(int i=0; i<3; i++){
			Work[i] = Available[i];
		}
		
		boolean flag = true;
		while(count < 3){
			if (flag) {
				System.out.println("进程 "+"  Alloction "+"  Need "+"   Work+Alloction ");
				flag = false;
			}
			for(int i=0; i<3; i++){
				//panduan
				if(Finish[i] == false && Need[i][0] <= Work[1] && Need[i][2] <= Work[2]){
					System.out.println("P"+i+" ");
					for(int k=0; k<3; k++){
						System.out.print(Work[k]+" ");
					}
					System.out.print("| ");
					for(int j=0; j<3; j++){
						Work[j] += Alloction[i][j];
					}
					Finish[i] = true; //当前进程能满足时
					S[count] = i ; //设置当前序列排号
					
					count++; //满足当前进程数加1
					for(int j=0; j<3; j++){
						System.out.print(Alloction[i][j]+" ");
					}
					System.out.print("|  ");
					
					for(int j=0; j<3; j++){
						System.out.print(Need[i][j]+" ");
					}
					System.out.print("|  ");
					
					for(int j=0;j<3; j++){
						System.out.print(Work[j]+" ");
					}
					System.out.println();
				}
			}
			circle++;//循环圈数加1
			//判断是否满足所有进程需要
			if(count == 3){
				System.out.print("此时存在一个安全序列：");
				for(int i=0; i<3; i++){
					System.out.print("P"+S[i]+" ");
				}
				System.out.println("故当前可分配！");
				break;				
			}
			//判断完成进程是否小于循环圈数
			if(count < circle){
				count = 5;
				System.out.println("当前进程处于不安全状态，所以不存在安全序列");
				break;
			}
		}
	}


	private void printSystemVariable() {
		System.out.println("此时资源分配量如下：");
		System.out.println("进程 "+" Max "+" Alloction "+" Need "+"   Available");
		for(int i=0; i<3; i++){
			System.out.println("P"+i+"  ");
			for(int j=0; j<3; j++){
				System.out.print(Max[i][j]+"  ");
			}
			System.out.print("|  ");
			
			for(int j=0; j<3; j++){
				System.out.print(Alloction[i][j]+"  ");
			}
			System.out.print("|  ");
			
			for(int j=0; j<3; j++){
				System.out.print(Need[i][j]+"  ");
			}
			System.out.print("|  ");
			if(i == 0){
				for(int j=0; j<3; j++){
					System.out.println(Available[j]+"  ");
				}
			}
			System.out.println();
		}
	}

	//设置已分配资源矩阵
	private void setAlloction() {
		System.out.println("请设置各进程分配矩阵Alloction：");
		for(int i=0; i<3; i++){
			System.out.println("请输入进程"+i+"的分配资源量：");
			for(int j=0; j<3; j++){
				Alloction[i][j] = scanner.nextInt();
			}
		}
		System.out.println("Available=Available-Allocation");
		System.out.println("Need = Max - Alloction");
		//设置Alloction
		for(int i=0; i<3; i++){
			for(int j=0; j<3; j++){
				Available[i]= Available[i] - Alloction[j][i];
			}
		}
		//need
		for(int i=0;i<3;i++){
			for(int j=0; j<3; j++){
				Need[i][j] = Max[i][j] -Alloction[i][j];
			}
		}
	}
    

    
    
}
